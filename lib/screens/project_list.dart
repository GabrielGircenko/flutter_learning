...
import 'package:priority_keeper/screens/task_list.dart';
...
class ProjectListState extends State<ProjectList>
            ...
            child: new Column(
              ...
              children: <Widget>[
                ...
                getKeepLikeListView(
                    ...
                    _getCheckedProjectListCount(),
                    checkedProjectControllers,
                    ScreenType.projects),
              ],
            ),
          ),
  ...
  void itemClicked(CheckedItemState state, int position) async {
    bool result =
      ...
      return TaskList(
          state.isChecked
              ? checkedProjectList[position]
              : projectList[position],
          state.isChecked
              ? checkedProjectList[position].title
  ...
  @override
  void delete(
      BuildContext context, Project project, CheckedItemState state) async {
    int result =
        await databaseHelper.deleteProject(state.isChecked, project.projectId);
    if (result != 0) {
  ...
  void updateUncheckedListView() {
    ...
    dbFuture.then((database) {
      Future<List<Project>> projectListFuture =
          databaseHelper.getProjectList(false);
  ...
  void updateCheckedListView() {
    ...
    dbFuture.then((database) {
      Future<List<Project>> projectListFuture =
          databaseHelper.getProjectList(true);
      projectListFuture.then((projectList) {
        setState(() {
          ...
          for (int i = 0; i < this.checkedProjectList.length; i++) {
            this.checkedProjectControllers.add(TextEditingController(
                  text: checkedProjectList[i].title != null
                      ? checkedProjectList[i].title
                      : "",
                ));
          }
        });
  ...
  void updateTitle(CheckedItemState state, int position) {
    if (state.isChecked) {
      checkedProjectList[position].title =
          checkedProjectControllers[position].text;
    } else {
  ...
  void save(BuildContext context, CheckedItemState state, ActionType action,
    ...
    if (_formKey.currentState.validate()) {
      ...
      if (projectList[position] != null &&
        ...
        if (state.isChecked) {
          result =
              await databaseHelper.updateProject(checkedProjectList[position]);
        } else {
        ...
        if (action == ActionType.check || action == ActionType.uncheck) {
          result *=
              await databaseHelper.updateProjectPositionsAfterOnCheckedChanged(
